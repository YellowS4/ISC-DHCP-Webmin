...
$manquant="";
	...
	if(isset($_POST['ajouter']) && $_POST['ajouter']==="Ajouter"){ //On ajoute la nouvelle configuration
		if(!isset($_POST['conf_contenu']) || $_POST['conf_contenu']=="" || !isset($_POST['interface']) || $_POST['interface']=="" || !isset($_POST['nom']) || $_POST['nom']=="" || !isset($_POST['actuelle']) || $_POST['actuelle']==""){ //On verifie tous les parametre
			...
			if(isset($erreur) && $erreur!=""){
				...
				echo 'Ajout effectue';
				$resultats=$connex->exec("INSERT INTO dhcp_test (conf_contenu, interface, nom_conf, date_creation, conf_actuelle) VALUES ('".$_POST['conf_contenu']."','".$_POST['interface']."','".$_POST['nom']."','".date("Y-m-d")."','".$_POST['actuelle']."'); ");
		...
		if(!isset($_POST['conf_contenu'])  || $_POST['conf_contenu']=="" || !isset($_POST['interface']) || $_POST['interface']=="" || !isset($_POST['nom']) || $_POST['nom']==""  || !isset($_POST['actuelle']) || $_POST['actuelle']==""){ //On verifie tous les parametre
			$manquant="veuillez remplir tous les champs";
			echo $manquant;
			echo '<form method="POST"><input type="submit" value="retour" name="affichage_simple"></form>';
		}else{
		...
		<form method="POST" action=""> 
			...
			<?php
	}elseif((isset($_POST['afficher_tout']) && $_POST['afficher_tout']==="Affichage détaillé") || (isset($_GET['id']) && $_GET['id']!=="" && !isset($_POST['affichage_simple'])) || (isset($_POST['selectionner']) && $_POST['selectionner']==="selectionner") || (isset($_POST['trie']) && $_POST['trie']==="trié")){
		//Tri
		...
		print '<form method="POST" action=""><input type="submit" name="afficher_tout" value="Affichage détaillé"> </form>';
	}
?>
</article>
